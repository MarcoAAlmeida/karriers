<template>
  <UModal v-model:open="open" :title="modalTitle" :ui="{ width: 'sm:max-w-lg' }">
    <template #content>
      <div data-testid="strike-detail-modal" class="p-4 space-y-4">

        <!-- No data guard -->
        <div v-if="!plan" class="text-sm text-slate-500 text-center py-4">
        ...
        <template v-else>
          ...
          <div class="flex items-center gap-2 flex-wrap">
            <UBadge :color="missionColor" variant="soft" size="sm" class="uppercase tracking-wide">
              {{ plan.mission }}
            </UBadge>
            <UBadge :color="statusColor" variant="subtle" size="sm">
              {{ plan.status }}
            </UBadge>
            <UBadge :color="plan.side === 'allied' ? 'info' : 'error'" variant="subtle" size="sm">
          ...
          <UCard :ui="{ body: { padding: 'px-4 py-3' } }">
            <dl class="space-y-1.5 text-sm">
              <div class="flex gap-2">
                <dt class="text-slate-500 w-24 shrink-0">Squadrons</dt>
                <dd class="text-slate-200">{{ squadronNames || '—' }}</dd>
              </div>
              <div class="flex gap-2">
                <dt class="text-slate-500 w-24 shrink-0">Origin</dt>
                <dd class="text-slate-200">{{ originCarrierName || '—' }}</dd>
              </div>
              <div class="flex gap-2">
                <dt class="text-slate-500 w-24 shrink-0">Target</dt>
                <dd class="text-slate-200">{{ targetLabel }}</dd>
              </div>
              <div class="flex gap-2">
                <dt class="text-slate-500 w-24 shrink-0">Launched</dt>
                <dd class="font-mono text-slate-300">{{ formatTime(plan.launchTime) }}</dd>
              </div>
              <div v-if="plan.eta" class="flex gap-2">
                <dt class="text-slate-500 w-24 shrink-0">{{ strikeResult ? 'Resolved' : 'ETA' }}</dt>
          ...
          <!-- Aircraft summary -->
          <UCard :ui="{ body: { padding: 'px-4 py-3' } }">
            <p class="text-[10px] uppercase tracking-widest text-slate-500 mb-2">Aircraft</p>
            <div class="flex gap-6 text-sm">
              <div class="text-center">
                <p class="text-slate-400 text-xs mb-0.5">Lost</p>
                <p class="text-red-400 font-semibold text-base tabular-nums">{{ plan.aircraftLost }}</p>
              </div>
              <div v-if="strikeResult" class="text-center">
                <p class="text-slate-400 text-xs mb-0.5">Returning</p>
                <p class="text-green-400 font-semibold text-base tabular-nums">{{ strikeResult.aircraftReturning }}</p>
              </div>
              <div v-if="strikeResult?.airCombat" class="text-center">
                <p class="text-slate-400 text-xs mb-0.5">Lost to CAP</p>
                <p class="text-orange-400 font-semibold text-base tabular-nums">{{ strikeResult.airCombat.attackerLosses }}</p>
              </div>
              <div v-if="strikeResult" class="text-center">
                <p class="text-slate-400 text-xs mb-0.5">Lost to flak</p>
                <p class="text-orange-400 font-semibold text-base tabular-nums">{{ strikeResult.flakLosses }}</p>
              </div>
            </div>
          </UCard>

          <!-- Combat results (resolved only) -->
          <UCard v-if="strikeResult" :ui="{ body: { padding: 'px-4 py-3' } }">
            ...
            <div v-if="strikeResult.hits.length" class="space-y-1 mb-3">
              <div
              ...
              >
                ...
                <span class="text-slate-300">
                  ...
                  <span v-if="hit.firesStarted" class="text-orange-400"> · {{ hit.firesStarted }} fire{{ hit.firesStarted !== 1 ? 's' : '' }}</span>
                </span>
              </div>
            </div>
            <div v-if="strikeResult.narrative.length" class="space-y-0.5 border-t border-slate-800 pt-2">
              <p
