<template>
  ...
  >
    ...
    <Transition name="fade">
      ...
      >
        <div class="font-semibold text-sky-300">{{ hoveredPlan.squadronName }}</div>
        <div class="text-slate-400 capitalize">{{ hoveredPlan.mission }} · {{ hoveredPlan.status }}</div>
        <div v-if="hoveredPlan.target" class="text-slate-400">Target {{ hoveredPlan.target }}</div>
        <div v-if="hoveredPlan.eta" class="text-slate-400">ETA {{ hoveredPlan.eta }}</div>
      </div>
    </Transition>
    ...
    >
      <div class="px-2 py-1 text-xs text-slate-400 border-b border-slate-700 bg-slate-900">Select squadron</div>
      <button
