  ...
  >
    ...
    <div class="flex items-center justify-between px-3 pt-3 pb-2">
      <div class="flex items-center gap-2 min-w-0">
        ...
        <UBadge
          :color="tg.side === 'allied' ? 'info' : 'error'"
          variant="subtle"
          size="sm"
        >
      ...
      <UButton
        ...
        color="neutral"
        variant="ghost"
        @click="mapStore.selectTaskGroup(null)"
      />
    </div>
    ...
    <div class="grid grid-cols-3 gap-1 px-3 pb-2 text-xs">
      <div class="bg-gray-900 rounded px-2 py-1">
        <p class="text-gray-500 uppercase tracking-wide text-[10px]">
          Order
        </p>
        <p class="text-gray-200 uppercase font-medium">
          {{ tg.currentOrder }}
        </p>
      </div>
      <div class="bg-gray-900 rounded px-2 py-1">
    ...
    <div class="border-t border-gray-800">
      ...
      <div class="max-h-44 overflow-y-auto divide-y divide-gray-800/50">
        ...
        >
          ...
          >{{ ship.name }}</span>
          <span
            :class="hullColor(hullPct(ship))"
            class="tabular-nums w-8 text-right"
          >
            {{ hullPct(ship) }}%
          </span>
          <span
            :class="ship.fires > 0 ? 'text-orange-400' : 'text-gray-600'"
            class="w-4 text-center"
          >
            {{ ship.fires > 0 ? ship.fires : '—' }}
          </span>
          <UBadge
            :color="shipStatusColor(ship.status)"
            variant="subtle"
            size="xs"
          >
            {{ ship.status }}
    ...
    <div
      v-if="tg.side === 'allied'"
      class="flex gap-2 p-3 border-t border-gray-800"
    >
      <UButton
        label="Order"
        size="xs"
